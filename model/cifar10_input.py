...
def _get_images_labels(batch_size, split, data_dir, distords=False):
  """Returns Dataset for given split."""
  dataset = tfds.load(name='cifar10', split=split, data_dir=data_dir)
...
def distorted_inputs(mode, batch_size, data_dir):
  ...
  """
  if mode=="train":
    return _get_images_labels(batch_size, tfds.Split.TRAIN, data_dir, distords=True)
  else:
    return _get_images_labels(batch_size, tfds.Split.TEST, data_dir, distords=True)
